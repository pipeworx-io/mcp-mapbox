...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Mapbox');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    description: '"What\'s at [lat,lng]" / "reverse geocode coordinates" / "what address is at these coords" / "what place is at this GPS point" — convert longitude / latitude into the nearest street address, neighborhood, city, region, or country via Mapbox\'s global geocoder. Works worldwide.',
  ...
  },
  {
    ...
    description: '"Distance matrix" / "drive times between multiple points" / "travel times grid" / "all-to-all distances" — N×M distance and duration matrix between many coordinates. Use for traveling-salesman, delivery routing, nearest-warehouse, or "which of these N stores is closest to each of these M customers" questions.',
    inputSchema: {
      type: 'object',
      properties: {
        coordinates: { type: 'array', items: { type: 'array', items: { type: 'number' } } },
  ...
  {
    name: 'map_matching',
    description: '"Snap GPS trace to roads" / "clean up noisy GPS coordinates" / "match track to road network" — takes a sequence of GPS points and returns the most likely path along the road network. Use for vehicle telematics, fitness-tracker cleanup, or "what road did this driver actually take" questions.',
    inputSchema: {
      type: 'object',
      properties: {
        coordinates: { type: 'array', items: { type: 'array', items: { type: 'number' } } },
  ...
  {
    name: 'tilequery',
    description: 'Query features inside a Mapbox vector tile around a coordinate — what POIs / roads / boundaries / buildings are within a radius of a point. Specialized GIS query against a specific tileset_id; most users want geocode_reverse or directions instead.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: '"Static map image of [location]" / "embed a map of [coords]" / "screenshot a map" / "thumbnail map" — generates a Mapbox static-tile image URL for embedding in slack, docs, emails, or dashboards. Pass style + coords + zoom + width/height; returns a fetchable PNG URL. No client-side JS or interactivity required.',
    inputSchema: {
      type: 'object',
      properties: {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!apiKey) throw new Error('Mapbox requires an API key (its access token): pass your key as the _apiKey argument (free at https://account.mapbox.com/access-tokens/).');
  const get = async (url: string) => {
    ...
    if (!res.ok) throw await httpError(res, 'Mapbox');
    return res.json();
  };
